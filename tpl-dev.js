...
function openPocPhasePopup() {
  ...
  $('modal-body').innerHTML = `<div class="jrny">${rows}
    <div class="mini" style="margin-top:4px">P1·P2 = 기획·제작 관제 · P3~P5 = 평가 관제 — 클릭하면 해당 단계 화면으로 전환 · 종합 클리어엔 선택 단계의 클리어 항목이 표시됩니다</div></div>`;
  $('modal-back').classList.add('open');
}

/* 선택 단계의 클리어 항목 — lifecycle[].clear, 미정의(최종 단계)면 게이트 기준 전체 */
function pocPhaseClearOpts(C, view, cur) {
  const lc = C.lifecycle || [];
  const s = lc[view] || {};
  const own = (s.clear && s.clear.length) ? s.clear : null;
  return {
    title: `종합 클리어 — ${own ? `P${view + 1} 클리어 항목` : '게이트 기준'}`,
    criteria: own || undefined,
    sub: pocPhaseTile(C, view, cur),
  };
}
